...
发布运维资格不属于 `npm test`，并且会安装/卸载目标端组件。只能在已授权、可销毁 Linux 环境运行 [`acceptance/operational/qualify-host.sh`](../acceptance/operational/qualify-host.sh)，随后用旧 schema 数据库副本执行 `npm run qualify:operational`。控制端会验证迁移前备份、旧任务读取、回退、重新迁移及 Evidence 离线导出摘要；完整命令和破坏性路径见 [`acceptance/operational/README.md`](../acceptance/operational/README.md)。
...
| `protocolV2.externalIntelBudget` | `100 / 1000 / 300000ms` | 外部威胁情报的调用数、IOC 数与墙钟预算。 |
...
| `webshell.modifiedWithinHours` | `168` | 任务未显式提供时的 Web 候选文件时间窗；GUI 创建任务时选择的时间窗优先。 |

`schemaVersion: 1` 配置会经显式结构迁移为 v2 预算分组。旧检测工具的细分上限、本地 YARA/Probe 路径及旧自动情报开关只作为迁移输入，加载后会被删除；其他未知字段直接拒绝，避免形成“可保存但不生效”的配置。生产运行时不保留 v1 Helper 协议分支。
...
npm run model:check -- --config ./config/local.yaml
```
...
# 静态检查，不请求 API
npm run model:check -- --config ./config/deepseek.yaml

# 发起一次小型真实请求，验证认证、流式响应和 Tool Call
npm run model:smoke -- --config ./config/deepseek.yaml

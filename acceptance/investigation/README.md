...
正式清单必须设置 `evaluationMode=BLIND_RELEASE`，并声明真值归档 SHA-256、冻结时间、互不相同的维护者与运行者，以及真值未进入目标授权、Helper 或模型上下文的隔离事实。解析器会拒绝少于 100 个恶意首跑、100 个良性首跑或没有独立受限首跑的总体，也会拒绝低于 95% 的采集覆盖率、恶意场景发现率或证据保全率、高于 5% 的良性误报率和非完整固定提交。恶意场景发现率只计入命中真值主体、引用该主体真值 Fact 且类别正确的有效风险结论；上述字段是可审计声明，真值归档和人员独立性仍须由发布方外部复核。

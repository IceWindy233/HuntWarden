...
- **五平台与业务负载已完成候选同提交验收，但稳定发布仍被阻断。** `0.3.0-beta.2` 候选在 Ubuntu 24.04 ARM64/x86_64、Debian 12 systemd x86_64、Rocky 9.8 x86_64 SELinux Enforcing、Amazon Linux 2023 x86_64 通过正式平台执行器；ARM64 为 Multipass，其余为本机 QEMU/TCG，不外推原生 x86_64 性能。RuoYi-Vue 原版隔离业务完成 12 次 Attach、2,497 次负载请求零失败；运维安装/升级/卸载、旧库迁移回退和 Evidence 导出通过。执行者为维护者委托的 AI/automation，不是第三方或生产用户背书。本次 Docker 14/14、GUI 16/16 和规模负载通过；真实 Provider 联合任务仍 FAIL，独立盲测未执行。证据、候选提交和边界见 [`docs/支持与验收说明.md`](docs/支持与验收说明.md)。

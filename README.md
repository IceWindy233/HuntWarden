...
- **五平台与业务负载已完成候选同提交验收，但稳定发布仍被阻断。** `0.3.0-beta.2` 资格候选在 Ubuntu 24.04 ARM64/x86_64、Debian 12 systemd x86_64、Rocky 9.8 x86_64 SELinux Enforcing、Amazon Linux 2023 x86_64 通过正式平台执行器；ARM64 为 Multipass，其余为本机 QEMU/TCG，不外推原生 x86_64 性能。RuoYi-Vue 原版隔离业务完成 12 次 Attach、2,497 次负载请求零失败；运维安装/升级/卸载、旧库迁移回退和 Evidence 导出通过。执行者为维护者委托的 AI/automation，不是第三方或生产用户背书。GitHub Prerelease [`v0.3.0-beta.2`](https://github.com/IceWindy233/HuntWarden/releases/tag/v0.3.0-beta.2) 已从合并提交重新构建并发布，但资格原件绑定预合并候选，不能改绑到发布提交；真实 Provider 联合任务仍 FAIL，独立盲测未执行。完整边界见 [`docs/支持与验收说明.md`](docs/支持与验收说明.md)。

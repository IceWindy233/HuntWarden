...
const javaRequest = "对当前 Tomcat 9/JDK 17 运行态执行 Java 内存马自主深度取证。复核运行时组件、精确 ClassLoader、类检查和完整字节码 Evidence；必须分别通过 propose_hypothesis 和 propose_actions 提交至少一个有事实依据、白名单且非重复的模型假设与调查动作，写入 Java 类别模型 Assessment，并确保最终结论引用实际类字节码 Evidence。jvm.class.inspect/dump 只接受已有 Fact 中的精确完整类名和可选 loader，不能用包前缀或通配符模拟全部已加载类枚举；能力不覆盖的范围应如实保留为 INCONCLUSIVE 限制。不得把 unknown Loader 或未检查范围解释为安全。";

...
describe("v2 HUMAN Assessment 与 Grant 撤销", () => {
  it("人工裁定只追加账本关系，Grant 撤销后既持久化又立即阻断已有工具闭包", async () => {
    ...
    const epoch: ScanEpoch = { epochId: "EPOCH-00000000-0000-4000-8000-000000000099", taskId: task.taskId, targetFingerprint: task.target.hostFingerprint, protocolVersion: 2, manifestVersion: "3.0.0", helperVersion: "3.0.0", reason: "INITIAL", status: "RUNNING", startedAt: new Date().toISOString() };
    ...
    const helper: HelperCapabilitiesV2 = { protocolVersion: 2, manifestVersion: "3.0.0", helper: { name: "helper", version: "3.0.0" }, namespaces: { file: { fields: ["path"], relations: [], verbs: ["enumerate"] }, account: { fields: ["uid"], relations: [], verbs: ["enumerate"] } }, matchers: [], probes: [], verbs: ["enumerate"], limits: { maxObjects: 10, maxOutputBytes: 4096, maxReadBytes: 1024, maxCollectBytes: 1024 } };

...
- `components`：Filter、Servlet、Listener、Valve、尽力枚举的 WebSocket Endpoint，以及已初始化 Spring DispatcherServlet 的 Controller 映射和 Interceptor。运行时对象会携带 `classLoaderId`、CodeSource、ProtectionDomain 与 Module 身份。
...
Spring MVC 枚举只调用已存在 ApplicationContext 和 HandlerMapping 的只读 getter，并受 Bean、映射和拦截器数量上限保护；未初始化 DispatcherServlet 或跨版本反射失败会明确标记 `NOT_PRESENT/PARTIAL`。探针不直接读取操作系统 TCP/UDP 连接，该部分由主机 Helper 完成。
